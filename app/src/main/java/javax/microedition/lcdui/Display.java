...
//		timer.scheduleAtFixedRate(new RunnableWrapper(new TickerPaintTask()), 0, Ticker.PAINT_TIMEOUT);
    ...
    public void setCurrent(final Displayable nextDisplayable) {
        ...
        if (nextDisplayable != null) {
            eventDispatcher.put(new ShowNotifyEvent(eventDispatcher, () -> {
                if (prevDisplayable != null) {
                    eventDispatcher.put(new HideNotifyEvent(eventDispatcher, () -> prevDisplayable.hideNotify(Display.this)));
                }
            ...
            JimmActivity.getInstance().minimizeApp();
        }
    }
