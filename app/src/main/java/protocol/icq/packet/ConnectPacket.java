...
public class ConnectPacket extends Packet {
    ...
    public byte[] toByteArray() {
        ...
        if (this.getType() == ConnectPacket.CLI_COOKIE) {
            ...
            // HELLO.PASSWORD
            buf.writeTLV(0x0003, FIXED_VERSION.getBytes());

            // HELLO.PASSWORD
            buf.writeTLV(0x0016, FIXED_UNKNOWN);

            // HELLO.PASSWORD
            buf.writeTLV(0x0017, FIXED_VER_MAJOR);

            // HELLO.PASSWORD
            buf.writeTLV(0x0018, FIXED_VER_MINOR);

            // HELLO.PASSWORD
            buf.writeTLV(0x0019, FIXED_VER_LESSER);

            // HELLO.PASSWORD
            buf.writeTLV(0x001a, FIXED_VER_BUILD);


            // HELLO.PASSWORD
            buf.writeTLV(0x0014, FIXED_VER_SUBBUILD);

            buf.writeTLV(0x000f, "ru".getBytes());

            buf.writeTLV(0x000e, "ru".getBytes());
